...
type pathNode struct {
	ResourcePathSegment
	methods  map[RestLiMethod]handler
...
func handle(rootNode *pathNode, res http.ResponseWriter, req *http.Request) {
	...
	if errRes, ok := err.(*stdtypes.ErrorResponse); ok {
		res.Header().Set(RestLiHeader_ErrorResponse, "true")
		responseBody = errRes
	...
	res.Header().Set(RestLiHeader_ProtocolVersion, RestLiProtocolVersion)
...
func (p *pathNode) receive(
	...
	restLiMethod := RestLiMethodNameMapping[ctx.Request.Header.Get(RestLiHeader_Method)]
		...
		if restLiMethod == Method_Unknown {
			...
			case http.MethodPost:
				return newErrorResponsef(nil, http.StatusBadRequest,
					"Header %q is required for POST requests", RestLiHeader_Method)
...
func newErrorResponsef(cause error, status int, format string, a ...any) (restlicodec.Marshaler, error) {
	if e, ok := cause.(*stdtypes.ErrorResponse); ok {
		return nil, e
	}
	if cause != nil {
		a = append(a, cause)
...
// exposed outside of this package and therefore should not be used. It is intended exclusively for testing purposes.
...
func registerMethod[RP ResourcePathUnmarshaler, QP restlicodec.QueryParamsDecoder](
	...
	p.methods[method] = func(
		...
		var queryParams QP
		if !stdtypes.IsEmptyRecord(queryParams) {
		...
		if _, ok := err.(*stdtypes.ErrorResponse); err != nil && !ok {
