...
func main() {
	...
	files := []*utils.CodeFile{
		{
			...
			PackagePath: utils.StdTypesPackage,
	...
	for _, f := range files {
		err := f.Write("protocol/"+utils.StdTypes, false)
		if err != nil {
			log.Panic(err)
		}
	}
...
func GenerateNewPagingContext() Code {
	...
	return Func().Id("NewPagingContext").
		...
		Add(resources.PagingContextIdentifier.Qual()).
		Block(
			Return(Add(resources.PagingContextIdentifier.Qual()).Values(Dict{
